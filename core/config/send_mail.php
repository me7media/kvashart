<?php
    ...
    $g_config['send_mail']['save_to_file_only'] = false;
    ...
    $g_config['send_mail']['from_email']  = "noreply@" . $_SERVER['SERVER_NAME']; // Email от имени которого отправляем. При использовании smtp, желательно чтобы был равен smtp_user (иначе можно попасть в спам)
    $g_config['send_mail']['from_name']   = "no-reply";          // Имя отправителя

    // SMTP
    $g_config['send_mail']['use_smtp']    = false;            // Использовать отправку через SMTP сервер или нет
    ...
    $g_config['send_mail']['smtp_user']   = "ACCOUNT@gmail.com"; // Email для авторизации на SMTP сервере.
    $g_config['send_mail']['smtp_pwd']    = "PASSWORD";          // Пароль для авторизации на SMTP сервере.
